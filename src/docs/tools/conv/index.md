...
アクセント記号は母音字の直後に `/` を入れる (`//` で第2アクセントになる).
...
一部の字母は後ろに `_` を加えることで別形になる (対応表を参照のこと).  
数字は1~9999の範囲のみ対応.
...
**ガイ式ラテン・アルファベット**と**キリル文字**間で相互変換する.  
ガイ式で補助記号の付く子音字は `/` を入力. なお č, ž, š は `ch`, `zh`, `sh` でも可能.  
例: ћ (ć) `c/` `t/`, ђ (đ) `d/`  
母音字の後に `/` を入れることでアクセント記号となる.  
例: á `a/`, à `a//`, ȃ `a///`, ȁ `a////`  
